...
-- Backfill previously-converted leads whose qualification data was dropped.
-- Only overwrite client columns that are currently NULL/empty.
UPDATE public.clients c
SET
  service_requested   = COALESCE(NULLIF(c.service_requested, ''), l.service_requested),
  budget              = COALESCE(NULLIF(c.budget, ''),            l.budget),
  timeline            = COALESCE(NULLIF(c.timeline, ''),          l.timeline),
  goals               = COALESCE(NULLIF(c.goals, ''),             l.goals),
  lead_quality        = COALESCE(c.lead_quality,                  l.lead_quality),
  ai_recommendation   = COALESCE(NULLIF(c.ai_recommendation, ''), l.ai_recommendation),
  lead_draft_reply    = COALESCE(NULLIF(c.lead_draft_reply, ''),  l.draft_reply),
  lead_draft_subject  = COALESCE(NULLIF(c.lead_draft_subject, ''),l.draft_subject),
  lead_score          = COALESCE(c.lead_score,
    CASE l.lead_quality
      WHEN 'High' THEN 'Hot'
      WHEN 'Medium' THEN 'Warm'
      WHEN 'Low' THEN 'Cold'
