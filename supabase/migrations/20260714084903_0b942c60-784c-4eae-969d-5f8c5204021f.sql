...
-- Add source column + partial unique index for acceptance-auto contracts
ALTER TABLE public.contracts ADD COLUMN IF NOT EXISTS source text;

CREATE UNIQUE INDEX IF NOT EXISTS contracts_one_acceptance_auto_per_proposal
  ON public.contracts (proposal_id)
  WHERE source = 'acceptance_auto' AND deleted_at IS NULL;

-- Replace client_portal_respond to return jsonb and safely claim a placeholder contract
