...
-- Backfill: append the two new fields (decision_maker, preferred_contact_method)
-- right before the extra_notes entry on the two previously-fixed rows, without
-- disturbing any other existing entries or saved responses.
WITH src AS (
  SELECT id,
         COALESCE(fields, '[]'::jsonb) AS f
...
rebuilt AS (
  SELECT src.id,
         (
           ...
           FROM (
             -- keep every non-extra_notes field in its original order
             SELECT elem, ord
             FROM jsonb_array_elements(src.f) WITH ORDINALITY AS t(elem, ord)
             WHERE elem->>'id' <> 'extra_notes'
             UNION ALL
             -- inject the two new fields just before extra_notes (or at end if missing)
             SELECT nf.nf_elem,
                    COALESCE(
                      (SELECT ord - 0.2 FROM jsonb_array_elements(src.f) WITH ORDINALITY AS t2(elem2, ord)
                       WHERE elem2->>'id' = 'extra_notes' LIMIT 1),
                      1e9
                    ) + nf.idx * 0.01
             FROM new_fields, LATERAL jsonb_array_elements(new_fields.nf) WITH ORDINALITY AS nf(nf_elem, idx)
             WHERE NOT EXISTS (
               SELECT 1 FROM jsonb_array_elements(src.f) AS existing(el)
               WHERE el->>'id' = nf.nf_elem->>'id'
             )
             UNION ALL
             -- re-append extra_notes at the very end (if it existed)
             SELECT elem, 1e10 + ord
             FROM jsonb_array_elements(src.f) WITH ORDINALITY AS t3(elem, ord)
             WHERE elem->>'id' = 'extra_notes'
           ) reordered
         ) AS new_fields_json
  FROM src
