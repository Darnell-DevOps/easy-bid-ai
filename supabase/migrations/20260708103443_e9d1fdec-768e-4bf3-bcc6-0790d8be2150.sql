...
-- Backfill: append the 3 new upload fields immediately after login_access on the two
-- previously-fixed rows, skipping any that already exist, without disturbing anything else.
WITH src AS (
  SELECT id, COALESCE(fields, '[]'::jsonb) AS f
  FROM public.onboarding_forms
  WHERE id IN ('9698a68c-2081-4fb6-bba4-14dfe84c44bd','6cfb66b2-1aab-4c5c-abc3-2f6f11bc8353')
),
new_fields AS (
  SELECT jsonb_build_array(
    jsonb_build_object('id','logo_upload','label','Upload your logo (if you have one)','type','file','accept','image/*','maxSizeMb',10,'group','Uploads'),
    jsonb_build_object('id','favicon_upload','label','Upload a favicon (small square icon, optional)','type','file','accept','image/*','maxSizeMb',5,'group','Uploads'),
    jsonb_build_object('id','business_description_upload','label','Upload a business description or brief document (optional)','type','file','accept','.pdf,.doc,.docx,.txt','maxSizeMb',15,'group','Uploads')
  ) AS nf
),
anchor AS (
  SELECT src.id,
         COALESCE(
           (SELECT ord FROM jsonb_array_elements(src.f) WITH ORDINALITY AS t(elem, ord)
            WHERE elem->>'id' = 'login_access' LIMIT 1),
           (SELECT COUNT(*) FROM jsonb_array_elements(src.f))
         )::numeric AS anchor_ord
  FROM src
),
rebuilt AS (
  SELECT src.id,
         (
           ...
           FROM (
             ...
             FROM jsonb_array_elements(src.f) WITH ORDINALITY AS t(elem, ord)
             UNION ALL
             SELECT nf_elem, (anchor.anchor_ord + idx * 0.01)::numeric
             FROM new_fields, LATERAL jsonb_array_elements(new_fields.nf) WITH ORDINALITY AS nf(nf_elem, idx), anchor
             WHERE anchor.id = src.id
               AND NOT EXISTS (
                 SELECT 1 FROM jsonb_array_elements(src.f) AS existing(el)
                 WHERE el->>'id' = nf_elem->>'id'
               )
           ) reordered
         ) AS new_fields_json
  FROM src
)
UPDATE public.onboarding_forms o
SET fields = r.new_fields_json
FROM rebuilt r
WHERE o.id = r.id;