...
-- Backfill the two tracked onboarding_forms rows to match new BASE_FIELDS.
-- Neither row has saved responses under logo_upload / favicon_upload / business_description_upload
-- (verified before running this migration), so all three legacy upload entries are removed and
-- replaced with the single reference_materials_upload field in the "Uploads" group.
