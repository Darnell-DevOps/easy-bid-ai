...
-- 1) Add acceptance_evidence column
ALTER TABLE public.proposals ADD COLUMN IF NOT EXISTS acceptance_evidence jsonb;

-- 2) Replace client_portal_respond to accept optional _evidence parameter
DROP FUNCTION IF EXISTS public.client_portal_respond(uuid, text, text);
...
-- 3) Harden public_get_proposal_by_id so it cannot leak acceptance_evidence back out publicly.
-- Keep SETOF proposals return shape (unchanged client-side typing) but strip the acceptance_evidence field.
...
AS $$
  SELECT (jsonb_populate_record(NULL::public.proposals, to_jsonb(p) - 'acceptance_evidence')).*
