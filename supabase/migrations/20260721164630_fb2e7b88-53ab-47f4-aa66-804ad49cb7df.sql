...
RETURNS TABLE (
  id uuid, user_id uuid, client_name text, company_name text, service_type text,
  proposal_content text, pricing_breakdown text, project_scope text, timeline text,
  created_at timestamptz, status text, sent_at timestamptz, viewed_at timestamptz,
  accepted_at timestamptz, paid_at timestamptz, rejected_at timestamptz,
  client_response_message text, amount_cents integer, currency text,
  client_paid boolean, payment_terms text, tax_rate numeric, tax_mode text
)
...
RETURNS TABLE (
  id uuid, user_id uuid, proposal_id uuid, contract_type text, title text,
  client_name text, client_email text, company_name text, body text, status text,
  signing_token text, signed_at timestamptz, amount_cents integer, currency text,
  countersigned_at timestamptz, countersigner_name text
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  WHERE c.signing_token = _token AND c.deleted_at IS NULL
  LIMIT 1;
$$;
...
RETURNS TABLE (
  id uuid, contract_id uuid, signer_name text, signer_email text, method text,
  signature_data text, signed_at timestamptz, signer_role text
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
...
BEGIN
  ...
  IF NOT has_function_privilege('anon', 'public.public_get_proposal_by_id(uuid)', 'EXECUTE')
    OR NOT has_function_privilege('anon', 'public.public_get_contract_by_token(text)', 'EXECUTE')
    OR NOT has_function_privilege('anon', 'public.public_get_contract_for_proposal(uuid)', 'EXECUTE')
    OR NOT has_function_privilege('anon', 'public.public_get_contract_signatures_by_token(text)', 'EXECUTE') THEN
  ...
  IF EXISTS (
    ...
    JOIN pg_proc procedure_definition ON procedure_definition.oid = target.function_oid
    CROSS JOIN LATERAL aclexplode(
      COALESCE(procedure_definition.proacl, acldefault('f', procedure_definition.proowner))
    ) AS exploded_acl
    WHERE exploded_acl.grantee = 0 AND exploded_acl.privilege_type = 'EXECUTE'
  ) THEN
