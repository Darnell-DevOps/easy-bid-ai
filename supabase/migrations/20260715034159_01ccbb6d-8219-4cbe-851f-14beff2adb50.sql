-- Add accepted-snapshot columns (Batch 4)
ALTER TABLE public.proposals
  ADD COLUMN IF NOT EXISTS accepted_proposal_content text,
  ADD COLUMN IF NOT EXISTS accepted_pricing_breakdown text,
  ADD COLUMN IF NOT EXISTS accepted_invoice_content text;

-- Update client_portal_respond to snapshot on accept, once, never overwrite.
DROP FUNCTION IF EXISTS public.client_portal_respond(uuid, text, text, jsonb);
  ...
  ELSIF _action = 'accept' THEN
    UPDATE public.proposals
       SET status = 'accepted',
           ...
           acceptance_evidence = COALESCE(acceptance_evidence, _evidence),
           -- One-time frozen snapshot of exactly what the client accepted.
           -- COALESCE ensures we NEVER overwrite an existing snapshot.
           accepted_proposal_content = COALESCE(accepted_proposal_content, proposal_content),
           accepted_pricing_breakdown = COALESCE(accepted_pricing_breakdown, pricing_breakdown),
           accepted_invoice_content = COALESCE(accepted_invoice_content, invoice_content)
     WHERE id = _proposal_id;
...
GRANT EXECUTE ON FUNCTION public.client_portal_respond(uuid, text, text, jsonb) TO anon, authenticated;