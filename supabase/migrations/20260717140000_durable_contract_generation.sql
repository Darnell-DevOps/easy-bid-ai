-- Durable state for acceptance-triggered contract generation.
...
-- Atomically claim one contract so browser retries and background retries cannot
-- generate the same agreement concurrently.
...
-- The public portal needs only the signable state, not internal retry errors.
