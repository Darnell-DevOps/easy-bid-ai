// Default professional templates for client-facing emails. Used as the
// starting point in the Templates editor and as a fallback at send time.
...
export interface ClientTemplate {
  ...
  body: string;
  cta_text: string;
  cta_url_var: string; // variable name (without braces) used as the CTA href
  sign_off: string;
  variables: string[];
}
...
// Legacy template names → client template key. Existing send sites pass these
// kebab-case names; the edge function uses this map to optionally apply a
// user customisation.
