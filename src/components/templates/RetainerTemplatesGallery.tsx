...
import {
  loadRetainerTemplateRows,
  mergeRetainerTemplates,
  templateToRetainerForm,
  type MergedRetainerTemplate,
  type RetainerTemplateRow,
} from "@/lib/retainer-templates";
import { formatMoney, intervalLabel } from "@/lib/retainers";
import RetainerTemplateEditorDialog from "@/components/templates/RetainerTemplateEditorDialog";

interface Props {
  onUseTemplate: (t: MergedRetainerTemplate) => void;
}

export default function RetainerTemplatesGallery({ onUseTemplate }: Props) {
  const { toast } = useToast();
  const [rows, setRows] = useState<RetainerTemplateRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [collapsed, setCollapsed] = useState(false);
  const [editorOpen, setEditorOpen] = useState(false);
  const [editorTarget, setEditorTarget] = useState<MergedRetainerTemplate | undefined>();
  const [editorInitial, setEditorInitial] = useState<any>(undefined);
  const [forceCreate, setForceCreate] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<MergedRetainerTemplate | null>(null);

  const reload = async () => {
    try {
      const { data: auth } = await supabase.auth.getUser();
      if (!auth.user) return;
      const r = await loadRetainerTemplateRows(auth.user.id);
      setRows(r);
    } catch (e: any) {
  ...
  const merged = useMemo(() => mergeRetainerTemplates(rows), [rows]);
  const builtins = merged.filter((m) => m.source === "builtin" || m.source === "builtin_override");
  const customs = merged.filter((m) => m.source === "custom" || m.source === "from_retainer");
  const defaultTpl = merged.find((m) => m.isDefault);

  const handleEdit = (t: MergedRetainerTemplate) => {
    setEditorTarget(t);
    setEditorInitial(undefined);
    setForceCreate(false);
    setEditorOpen(true);
  };
  const handleDuplicate = (t: MergedRetainerTemplate) => {
    setEditorTarget(t);
    setEditorInitial({ ...templateToRetainerForm(t), name: `${t.name} (copy)` });
    setForceCreate(true);
    setEditorOpen(true);
  };
  const handleCreate = () => {
  ...
  const handleResetBuiltin = async (t: MergedRetainerTemplate) => {
    if (!t.rowId) return;
    const { error } = await supabase.from("retainer_templates" as any).delete().eq("id", t.rowId);
    if (error) {
  ...
  const handleSetDefault = async (t: MergedRetainerTemplate) => {
    try {
      const { data: auth } = await supabase.auth.getUser();
      ...
      if (t.rowId) {
        ...
        await supabase.from("retainer_templates" as any).insert({
          user_id: userId,
          name: t.name,
          description: t.description,
          service_type: t.service_type,
          best_for: t.best_for,
          default_amount_cents: t.default_amount_cents,
          default_currency: t.default_currency,
          default_interval: t.default_interval,
          default_custom_days: t.default_custom_days,
      ...
      <CardContent className="p-4 sm:p-5 space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-foreground">Retainer templates</h2>
            {defaultTpl && (
...
interface GridProps {
  items: MergedRetainerTemplate[];
  onUse: (t: MergedRetainerTemplate) => void;
  onEdit: (t: MergedRetainerTemplate) => void;
  onDuplicate: (t: MergedRetainerTemplate) => void;
  onSetDefault: (t: MergedRetainerTemplate) => void;
  onResetBuiltin?: (t: MergedRetainerTemplate) => void;
  onDelete: (t: MergedRetainerTemplate) => void;
}
  ...
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {items.map((t) => {
        ...
        const isCustom = t.source === "custom" || t.source === "from_retainer";
        return (
          <Card
            key={t.rowId || t.id}
            role="button"
          ...
          >
            ...
            <CardContent className="p-4 pr-12">
              <div className="flex items-start gap-3">
                <div
                  className={`w-9 h-9 rounded-lg bg-gradient-to-br ${t.accent} flex items-center justify-center flex-shrink-0 shadow-sm`}
                >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 flex items-center gap-3 flex-wrap">
                    <span className="text-xs font-semibold text-foreground">
                      {formatMoney(t.default_amount_cents, t.default_currency)}
                      <span className="text-[10px] font-normal text-muted-foreground">
                        {" "}/ {intervalLabel(t.default_interval, t.default_custom_days).toLowerCase()}
                      </span>
                    </span>
                    {t.best_for && (
